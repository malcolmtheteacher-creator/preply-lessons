...
**The student produces 70%+ of the lesson's content.** The lesson provides scaffolding, prompts, models, and feedback — but the student does the thinking, writing, and speaking. Every tab must require active student production. If a tab is mostly reading/listening with no output task, it fails.
...
**10. SPACED REPETITION (Distributed Practice Across Time)**
Items learned in previous lessons return at optimal intervals to combat the forgetting curve.
- Mechanism: The forgetting curve shows that without review, we lose ~80% of new information within days. Spacing practice at increasing intervals (not cramming) produces superior long-term retention.
- Implementation: Every lesson includes a **Recall Zone** that retrieves 2-3 items from PREVIOUS lessons (not just from earlier in the current lesson). The Recall Zone uses research-backed spacing intervals: items return at +3, +7, +14, and +21 lessons after initial learning.
- Example: Lesson 10 recalls items from Lessons 3, 6, and 8 (not Lesson 9). Lesson 15 recalls from Lessons 4, 8, and 12. The spacing creates optimal retrieval difficulty.
- Context matters: When the user provides a lesson number (e.g., "this is lesson 8"), calculate which previous lessons to recall from using optimal spacing. If no lesson number is given, include a Recall Zone placeholder that prompts Malcolm to specify which items to spiral.
- For standalone lessons: Include a "Future Recall" note suggesting when this content should be reviewed (e.g., "These expressions should reappear in lessons +3, +7, +14 from today").
- Gold Standard: The Speaking Pathway course architecture (COURSE_ARCHITECTURE.md in gitsite) demonstrates full implementation of spaced repetition across 75 lessons. Study it as the reference model.
...
7. RECALL ZONE → Spaced retrieval from previous lessons (+3, +7, +14 intervals)
8. RETRIEVAL + METACOGNITION → Recall from memory + "What helped you learn that?" + "I can..." statement
```

Note: The Recall Zone (step 7) can be integrated flexibly within the lesson structure. Some archetypes place it near the end (after new content practice), others integrate it earlier. The key is that it MUST retrieve from PREVIOUS lessons, not just review the current lesson's content.
...
All archetypes MUST embed the 10 Non-Negotiables from the Pedagogy Engine. The structures below show WHERE each principle appears.
...
| 4 | Your Turn | 6 discussion cards — student produces 70%+ | STUDENT OUTPUT |
| 5 | Deep Dive | Timed speaking + elaboration challenge | ELABORATION |
| 6 | Recall Zone | Spaced retrieval from previous lessons | SPACED REPETITION |
| 7 | Retrieval Wrap-Up | Student reproduces key items from memory + "I can..." | RETRIEVAL AGAIN + METACOGNITION |
...
- 70%+ student speaking/writing time — teacher facilitates only
...
**Tab 6 - Recall Zone (Spaced Repetition):**
- Retrieves 2-3 items from PREVIOUS lessons (not just from this lesson)
- If lesson number is provided: Calculate optimal spacing (+3, +7, +14 lessons back)
  - Example: Lesson 10 recalls from Lessons 7, 3, and earlier
  - Example: Lesson 15 recalls from Lessons 12, 8, 4
- Use click-to-reveal: Show PROMPT first (e.g., "What expression did we learn for...?"), student attempts, THEN reveals the answer
- Include brief context: "In Lesson [X], we learned..."
- Dual coding: Visual cues to trigger memory (icons, colours from original lesson)
- If no lesson number: Include placeholder "Recall Zone - Malcolm to specify items from previous lessons"
- This combats the forgetting curve by spacing retrievals at research-backed intervals

**Tab 7 - Retrieval Wrap-Up + Metacognition:**
...
- **"I can..." statement:** End with a specific self-assessment — "I can use these 5 expressions to [specific ability]"
...
### Quality Gate
> "Would students lose track of time because the content is too interesting AND are they producing 70%+ of the output?"
...
| 6 | Speaking/Production | Real communication task — 70%+ student output | STUDENT OUTPUT |
| 7 | Recall Zone | Spaced retrieval from previous lessons | SPACED REPETITION |
| 8 | Retrieval Test | Final recall — no notes, reproduce the rule + examples + "I can..." | RETRIEVAL AGAIN |
...
- 70%+ student output — teacher only prompts/redirects
- No scaffolding visible — student must recall and apply independently

**Tab 7 - Recall Zone (Spaced Repetition):**
- Retrieves 2-3 grammar structures from PREVIOUS lessons
- If lesson number provided: Use optimal spacing (+3, +7, +14 lessons back)
  - Example: Lesson 12 recalls Present Simple (Lesson 5), Present Continuous (Lesson 8), and modals (Lesson 9)
- For each recalled item: Provide 2-3 quick exercises mixing old + new structures
- Click-to-reveal: Student attempts BEFORE seeing correct answers
- Dual coding: Include visual reminders (timelines, diagrams) from original lessons
- This prevents forgetting and strengthens long-term retention across the course

**Tab 8 - Retrieval Test + Metacognition:**
...
- **"I can..." statement:** "I can use [structure] to [specific communicative function]"
- Links back to learning intention: "We said you'd be able to [Y]. Can you?"
...
| 6 | Application | Real scenario, 70%+ student output | STUDENT OUTPUT |
| 7 | Recall Zone | Spaced retrieval from previous lessons | SPACED REPETITION |
| 8 | Retrieval Debrief | Reproduce the framework from memory + self-assess + "I can..." | RETRIEVAL AGAIN |
...
**Tab 7 - Recall Zone (Spaced Repetition):**
- Retrieves 2-3 frameworks/techniques from PREVIOUS lessons
- If lesson number provided: Use optimal spacing (+3, +7, +14 lessons back)
  - Example: If teaching STAR method in Lesson 10, recall PARADE (Lesson 3) and presentation structure (Lesson 6)
- For each recalled framework: Give 1 scenario requiring its application
- Student must identify WHICH framework to use (diagnosis, not just execution)
- Dual coding: Visual reminders of framework acronyms/structures from original lessons
- Interleaving: Mix frameworks so student must choose the right tool for each situation

**Tab 8 - Retrieval Debrief + Metacognition:**
...
- **"I can..." statement:** "I can apply [framework] to [specific performance context]"
- Links back to learning intention: "We said you'd be able to [Y]. Can you?"
...
| 6 | Full Practice | Apply ALL decisions — 70%+ student output | STUDENT OUTPUT |
| 7 | Recall Zone | Spaced retrieval from previous lessons | SPACED REPETITION |
| 8 | Retrieval Debrief | Reproduce decisions from memory + self-assess + "I can..." | RETRIEVAL AGAIN |
...
- 70%+ student output — no scaffolding
...
**Tab 7 - Recall Zone (Spaced Repetition):**
- Retrieves 2-3 exam strategies from PREVIOUS lessons
- If lesson number provided: Use optimal spacing (+3, +7, +14 lessons back)
  - Example: If teaching IELTS Writing Task 2 in Lesson 10, recall Task 1 strategies (Lesson 3) and Speaking Part 2 techniques (Lesson 7)
- For each recalled strategy: Provide 1 mini-task applying that strategy
- Student must recall the decision framework without looking back
- Dual coding: Visual reminders of Band descriptors, decision flowcharts from original lessons
- This builds cumulative exam mastery across the course
...
- **"I can..." statement:** "I can apply all 4 strategic decisions to achieve Band [X] in [exam task]"
- Links back to learning intention: "We said you'd be able to [Y]. Can you?"
...
Use clear, consistent file names that indicate the lesson's purpose and place in a sequence:

**For course sequences (Speaking Pathway model):**
```
sp_[level]_[number]_[topic].html

Examples:
sp_a2_01_buying_time.html
sp_b1_21_extended_thinking.html
sp_b2_45_review.html
sp_c1_74_impromptu_speaking.html
```

**For standalone lessons:**
```
[archetype]_[level]_[topic].html

Examples:
curious_b1_social_media_addiction.html
grammar_a2_present_perfect.html
skills_b2_interview_parade.html
exam_c1_ielts_task1.html
```

**For review/spiral lessons:**
- Append `_review` or `_spiral` to indicate spaced repetition focus
- Example: `sp_a2_05_review.html` (reviews lessons 1-4)

---

## SPEAKING SKILL DOMAINS (Reference Taxonomy)

When creating speaking-focused lessons, reference these six domains from the Speaking Pathway architecture:

### 1. FLUENCY & FLOW
Hesitation fillers, thinking time strategies, self-correction, chunking and rhythm

### 2. INTERACTION & RESPONSE
Active listening signals, clarification requests, turn-taking, building on what others say

### 3. DISCOURSE & STRUCTURE
Organizing monologues, sequencing, comparing/contrasting, cause and effect

### 4. OPINION & ARGUMENT
Expressing views, hedging certainty, supporting with reasons, acknowledging other views

### 5. DESCRIPTION & EXPLANATION
Describing people/places/objects, explaining processes, giving definitions, paraphrasing

### 6. REGISTER & APPROPRIACY
Formal vs informal, softening and politeness, idiomatic expression, cultural awareness

Use these domains to classify speaking techniques and ensure progressive coverage across a course. See `COURSE_ARCHITECTURE.md` in gitsite for full implementation.
...
- [ ] **Includes RECALL ZONE retrieving items from PREVIOUS lessons (spaced repetition)?**
- [ ] Ends with RETRIEVAL + one METACOGNITIVE question?
- [ ] **Includes "I can..." self-assessment statement?**
- [ ] **Student output ratio ≥ 70%?**
...
### Version 5.0-5.1 Failures (Missing Spaced Repetition)
- **CRITICAL OMISSION:** Spaced repetition was acknowledged in anti-patterns table as best practice, but NOT IMPLEMENTED in any archetype structure
- No Recall Zone mechanism for retrieving items from previous lessons
- No optimal spacing intervals (+3, +7, +14, +21)
- Each lesson was a standalone unit with no cross-lesson memory consolidation
- This violated the "Gold Standard #2" principle from Dunlosky et al. (2013) despite being explicitly mentioned in the research foundation
- Speaking Pathway implemented spaced repetition correctly, but the skill that generates lessons didn't include it — a failure of logical coherence between the skill and its exemplar
...
## SPEAKING PATHWAY AS GOLD STANDARD

The Speaking Pathway course (see `COURSE_ARCHITECTURE.md` in gitsite) represents the complete implementation of all 10 Non-Negotiables across a 75-lesson course. It demonstrates:
- Full spaced repetition system with Recall Zones in every lesson
- Optimal spacing intervals (+3, +7, +14, +21 lessons)
- "I can..." statements for self-assessment
- Six Speaking Skill Domains taxonomy
- Consistent 70% student output ratio
- Systematic file naming conventions

When in doubt about how to implement any principle, consult the Speaking Pathway as the reference model. The archetypes in this skill provide structural flexibility, but all lessons must comply with the same evidence-based principles that govern the Speaking Pathway.
...
- **v5.2: Spaced Repetition implementation (CRITICAL UPDATE)** — Added 10th Non-Negotiable (Spaced Repetition), Recall Zone component to all archetypes, "I can..." self-assessment statements, Speaking Skill Domains taxonomy, file naming conventions. Student output ratio updated to 70%+ for consistency with Speaking Pathway gold standard. Speaking Pathway (COURSE_ARCHITECTURE.md) referenced as complete implementation model. This update fixes the critical omission of spaced repetition mechanisms despite being acknowledged as Gold Standard #2 in research.
- Date: February 2026
