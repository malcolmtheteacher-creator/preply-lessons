...
## 4. The lesson shape — six tabs

🔁 **Warm-up** → 📖 **The Story** → 🗂️ **Words** → 🧩 **The Pattern** → 🗣️ **Your Turn** → 🎭 **Role Play**
...
- **🎭 Role Play** — the student steps INTO this chapter and acts it out with the teacher, using only this lesson's words + pattern (and earlier language). Two rounds, roles swapped; Round 2 changes the details so Round 1 can't be parroted. The student's lines are **cues** (*say you can't find your keys*), never written-out sentences. Every lesson at every stage is **built with this tab from the start** — it is not optional and not an afterthought.

Structure that must survive any clone: 6 `.panel` divs (one `class="panel active"`), 6 `.tab-btn` (one `.active`).
